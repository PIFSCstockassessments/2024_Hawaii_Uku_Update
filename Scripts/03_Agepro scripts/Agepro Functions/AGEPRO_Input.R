 ...
 GENERAL <- c(ProjStart, ProjStart + (NYears-1), MinAge, MaxAge, NSims, Nfleets, NRecr_models, Discards, set.seed)
  ...
  if (TimeVary[4] == 1) {
    ...
    if (UserSpecified[4] == 0) {
      ...
      for (i in 1:Nfleets){lines[[paste0("CatchWeight",i+Nfleets+3)]]<-paste(Catch_WAACV[i,-1], collapse = "  ")}
    }
  }
  ...
  ## Recruitment section
  lines$recr  <- "[RECRUIT]"
  lines$recr2 <- paste(Recruitment$general_rec_pars, collapse="  ")
  lines$recr3 <- paste(Recruitment$rec_models, collapse="  ")
  
  # Recruitment model(s) probabilities
  for (i in 1:NYears){ lines[[paste0("recr",i+3)]] <- paste(Recruitment$rec_model_probs, collapse="  ")}
  
  # Recruitment model(s) parameters
  for(rm in 1:n_rec_models){ # NOTE FROM Marc N.: The general multi-models structure is there, but have only fixed model 3 and 5.
    
    aRecMod  <- Recruitment$rec_models[[rm]]
    aRecPars <- Recruitment$rec_pars[[rm]] # Load the parameters for the model in the loop
    
    if (aRecMod %in% c(1, 4, 9, 15)) {
    ...
    if (aRecMod %in% c(3)) {
      RecObs                         <- SS_Out$RecruitmentObs %>% filter(Yr>=aRecPars$StartYr & Yr<=aRecPars$EndYr )
      RecObs                         <- RecObs$pred_recr
      lines[[paste0("Rmod",rm,"a")]] <- length(RecObs)
      lines[[paste0("Rmod",rm,"b")]] <- paste(RecObs,collapse=" ")
    }
