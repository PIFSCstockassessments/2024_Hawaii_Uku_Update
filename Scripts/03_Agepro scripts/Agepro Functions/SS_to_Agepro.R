...
SS_To_Agepro<-function(model.dir = "base",
                       script.dir = file.path("Scripts","03_Agepro scripts"),
                       #endyr = 2020,
                       TimeStep = "Year"){

require(tidyverse)
  
source(file.path(script.dir,"Agepro Functions","SS_outputforAGEPRO.R"))
...
# Alternative specification by Marc Nadon :

Steepness <- base.model$parameters[which(base.model$parameters$Label=="SR_BH_steep"),"Value"]
R0        <- base.model$derived_quants[which(base.model$derived_quants$Label=="Recr_Virgin"),"Value"]
SSB0      <- base.model$derived_quants[which(base.model$derived_quants$Label=="SSB_Virgin"),"Value"]
BH_Var    <- base.model$parameters[which(base.model$parameters$Label=="SR_sigmaR"),"Value"]^2  # IMPORTANT: need to square since Agepro takes Variance as a parameter

Out[["alpha"]] <- 4*Steepness*R0/(5*Steepness-1)
Out[["beta"]]  <- SSB0*(1-Steepness)/(5*Steepness-1) * 2  # IMPORTANT: the reason for beta x 2 is that Agepro SSB is in TOTAL biomass, not just FEMALE biomass, like SSB.
...
if(TimeStep=="Year"){
  ...
  Mat_Slope <- base.model$parameters[which(base.model$parameters$Label=="Mat_slope_Fem_GP_1"),"Value"]
  Mat_50    <- base.model$parameters[which(base.model$parameters$Label=="Mat50%_Fem_GP_1"),"Value"]
  LatAge    <- base.model$growthseries[which(base.model$growthseries$Yr==endyr&base.model$growthseries$Seas==1&
                                  base.model$growthseries$SubSeas==1),6:ncol(base.model$growthseries)]
  
  Out[["MatAtAge"]] <- 1 / (1 + exp(Mat_Slope*(LatAge-Mat_50)))
  ...
  Out[["NatMort_atAge"]]   <- base.model$Natural_Mortality %>%
                              slice(1) %>%
                              select(6:ncol(.))
  Out[["NatMort_atAgeCV"]] <- rep(0.01,Out$MaxAge)
